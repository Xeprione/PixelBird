...
public class PauseMenu extends Dialog {
	...
	public void render(Screen s) {
		...
		s.render(s.xOffset + 32, s.yOffset + s.height - 16, 5 + 32, color,
				0x06, 1);
		s.render(s.xOffset + s.width - 32, s.yOffset + 5, 5 + 32, color, 0x09,
				1);
		s.render(s.xOffset + s.width - 32, s.yOffset + s.height - 16, 5 + 32,
				color, 0x03, 1);
		...
		for (int x = 0; x < 11; x++) {
			for (int y = 0; y < 11; y++) {
				s.render(s.xOffset + 40 + x * 8, s.yOffset + 13 + y * 8,
						8 + 32, color, 0x00, 1);
				s.render(s.xOffset + 40 + x * 8, s.yOffset + 5, 7 + 32, color,
						0x00, 1);
				s.render(s.xOffset + 40 + x * 8, s.yOffset + s.height - 16,
						7 + 32, color, 0x03, 1);

				s.render(s.xOffset + 32, s.yOffset + 13 + y * 8, 6 + 32, color,
						0x00, 1);
